...
% regularizaion of the cost function
...
J = J + (lambda / (2 * m)) * ( sum( sum( Theta1_new.^2 ) ) + sum( sum( Theta2_new.^2 ) ) )
...
for t = 1:m
    ...
    a2 = [1; a2];       % 26 x 1 (add bias)
    z3 = Theta2 * a2;  % 10 x 26 * 26 x 1
    a3 = sigmoid(z3);   % 10 x 1
    
    
    % step 2
    delta_3 = (a3 - y_new(:,t)) % 10 x1
    
    
    % step 3
    z2 = [1; z2]
    delta_2 = (Theta2' * delta_3) .* sigmoidGradient(z2)    % 26 x 10 * 10 x 1 = 26 x 1


    % step 4
    delta_2 = delta_2(2:end) % skip delta_2(0)
...
Theta1_grad(:, 2:end) = Theta1_grad(:, 2:end) + ((lambda/m) * Theta1(:, 2:end)); % for all j > 0
Theta2_grad(:, 2:end) = Theta2_grad(:, 2:end) + ((lambda/m) * Theta2(:, 2:end)); % for all j > 0
